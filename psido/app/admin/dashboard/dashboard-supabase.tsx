...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && !localStorage.getItem("psido_admin")) {
      router.push("/admin");
    }
  }, [router]);
...
[O      city: c.city, category: c.category, plan: c.plan, status: c.status,
  ...
  const save = async () => {
    if (!form.shop_name || !form.phone) return;
    setSaving(true);
    try {
      if (editClient) {
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        @keyframes pulse{0%,100%{opacity:1;box-shadow:0 0 0 0 rgba(0,255,136,0.4)}50%{opacity:0.7;box-shadow:0 0 0 4px rgba(0,255,136,0)}}
        ...
        .toast{position:fixed;bottom:32px;left:50%;transform:translateX(-50%);background:#06080F;border:1px solid var(--border);color:var(--text);padding:14px 28px;font-family:'Orbitron',monospace;font-size:12px;letter-spacing:0.1em;z-index:99999;animation:popUp 0.3s ease both;box-shadow:0 8px 40px rgba(0,255,136,0.15);}
          ...
          {tab === "clients" && (
            <>
              ...
              <div className="table-wrap">
                {loading ? (
                  <div className="loading-state"><div className="loading-spinner" /><div className="loading-txt">Loading from database...</div></div>
                ) : filtered.length === 0 ? (
                  <div className="empty-state"><div className="empty-icon">🏪</div><div className="empty-txt">{search ? "No clients match your search" : "No clients yet — add your first one!"}</div></div>
                ) : (
